...
class AbstractBoard:
    ...
    def populate_board(self):
        raise NotImplementedError()  # 추상클래스를 이렇게 한거지
...
class CheckersBoard(AbstractBoard):
    ...
    def populate_board(self):  # 팩터리 메서드
        for x in range(0, 9, 2):
            for y in range(4):
                column = x + ((y + 1) % 2)
                for row, color in ((y, "black"), (y + 6, "white")):
                    self.board[row][column] = create_겟piece("draught",
                            color)  # create_piece() 팩터리 함수 사용
...
def create_piece(kind, color):
    ...
    # eval()로 클래스 인스턴스 생성. Using eval() is risky: 어떤 것이든 생성할 수 있기에
...
class {}(Piece):
    ...
    # exec()로 클래스 생성(글자와 명칭을 알면). Using exec() is risky: eval()보다 잠재 위험 더 큼
