...
"""
미흡한 점
- 팩터리는 상태 정보를 갖고 있을 필요가 없음. 실제 팩터리 인스턴스를 생성할 필요가 없음
- SvgDiagramFactory와 DiagramFactory 거의 동일. 불필요한 중복 존재
- 최상위 네임스페이스에 모든 클래스가 모여있음 but 접근은 두 개만 함(DiagramFactory, SvgDiagramFactory)완
"""
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == "-P":  # For regression testing
        ...
        return
    textFilename = os.path.join(os.getcwd(), "diagram.txt")  # 기존 예시인 .join(tempfile.gettempdir(),...) 에서 수정
...
class SvgDiagramFactory(DiagramFactory):
    # 각 메서드가 DiagramFactory와 다른 점은 반환 객체 뿐
