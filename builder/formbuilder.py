...
def main():
    ...
    htmlFilename = os.path.join(os.getcwd(), "login.html")
    htmlForm = create_login_form(HtmlFormBuilder())  # 생성 함수에 빌더 객체를 인자로 전달해 호출
    with open(htmlFilename, "w", encoding="utf-8") as file:
        file.write(htmlForm)
    print("wrote", htmlFilename)

    tkFilename = os.path.join(os.getcwd(), "login.py")
...
class HtmlFormBuilder(AbstractFormBuilder):
    ...
    def add_title(self, title):
        super().add_title(escape(title)) # escape: 기호들을 string으로 전처리
