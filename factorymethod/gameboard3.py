...
def make_new_method(char): # 매번 새로운 메서드를 만들기 위해 피룡
    def new(Class): # Can't use super() or super(Piece, Class): super() 함수가 접근할 클래스가 아직 없어
...
for code in itertools.chain((0x26C0, 0x26C2), range(0x2654, 0x2660)):
    ...
    setattr(sys.modules[__name__], name, Class) # Can be done better! eval()이나 exec()활용하지 않고 안정하게 동적 생성 가능
...
class AbstractBoard:
    ...
    def create_piece(self, kind, color):
        return AbstractBoard.__classForPiece[kind, color]()  # 2-튜플 조회 후 클래스 호출 후 인스턴스 반환
            ...
            for y in range(4):
                ...
                for row, color in ((y, BLACK), (y + 6, WHITE)):
                    self.board[row][column] = self.create_piece(DRAUGHT,
                            color)  # 문자열 대신 상수 값을 사용해 말과 색 지정
