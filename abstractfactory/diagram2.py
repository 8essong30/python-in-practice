...
"""
diagram1.py 보완
- Diagram, Rectangle, Text 클래스를 내부 클래스로
    -> 클래스 이름 충돌이 없으므로 이름을 동일하게 만들 수 있음 (SvgDiagramFactory.Diagram, DiagramFactory.Diagram)
- 상수 값 내장
=> 최상위 네임스페이스에 main(), create_diagram(), DiagramFactory, SvgDiagramFactory 만 존재
"""
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == "-P": # For regression testing
        ...
        return
    textFilename = os.path.join(os.getcwd(), "diagram2.txt")
    svgFilename = os.path.join(os.getcwd(), "diagram2.svg")

    txtDiagram = create_diagram(DiagramFactory) # 인스턴스 생성 안함 클래스 메서드의 내부 클래스 반환
...
class DiagramFactory:
    ...
    def make_diagram(Class, width, height):
        return Class.Diagram(width, height) # 내부 클래스로 생성
    ...
    def make_text(Class, x, y, text, fontsize=12):
        return Class.Text(x, y, text, fontsize)  # DiagramFactory.Text 객체 반환
...
font-family="sans-serif" font-size="{fontsize}">{text}</text>"""
    ...
    class Diagram:
        ...
        def __init__(self, width, height):
            ...
            self.diagram = [SvgDiagramFactory.SVG_START.format(**locals())] # 내부 상수에 접근
